...
for dir in dirlist:
    ...
    print(f"{fix}, train_acc var={train_acc.var():.4f}, train_auc var={train_auc.var():.4f}, test_acc var={test_acc.var():.4f}, test_auc var={test_auc.var():.4f}")
    ...
    for dropout in [0.0, 0.1]:
        ...
        for size, step in [(4800, 4800), (480, 180)]:
            for node in [15, 25, 50]:
                train_acc, test_acc, train_auc, test_auc = loadLog(f"CNN_node={node}_size={size}_step={step}_rho=0.100000_dropout={dropout}_epochs=64_num_iter=150_lr=0.001_channel={channel}")
        ...
        for size, step in [(480, 180)]:
            for node in [15, 25, 50]:
                train_acc, test_acc, train_auc, test_auc = loadLog(f"CNN_node={node}_size={size}_step={step}_rho=0.100000_dropout={dropout}_epochs=64_num_iter=150_lr=0.001_choose_channel={channel}")
                train_acc_list.append(train_acc.mean())
                test_acc_list.append(test_acc.mean())
                train_auc_list.append(train_auc.mean())
                test_auc_list.append(test_auc.mean())
...
for channel in [1, 2, 4]:
    for dropout in [0.0, 0.1]:
        ...
        for size, step in [(4800, 4800), (480, 180)]:
            for node in [15, 25, 50]:
                dir = f"CNN_node={node}_size={size}_step={step}_rho=0.100000_dropout={dropout}_epochs=64_num_iter=150_lr=0.001_channel={channel}"
                train_acc = loadFile(os.path.join(dir, "train_acc.log"))[:, -1]
                test_acc = loadFile(os.path.join(dir, "test_acc.log"))[:, -1]
                train_auc = loadFile(os.path.join(dir, "train_auc.log"))[:, -1]
                test_auc = loadFile(os.path.join(dir, "test_auc.log"))[:, -1]
